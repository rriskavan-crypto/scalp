...
//|
//|   Oltin — dam olish kunlari yopiq, London/NY seansida faol (4h)
//|   Savdo vaqti : Dushanba-Juma, juma 19:00 UTC dan keyin yangi savdo yo'q
//|   tipik spread 0.3 @ narx 2,650 -> xarajat ~0.023 R
//|
...
#property description "ScalpKit XAU/USD (oltin) — swing / trendni kuzatish (M15-D1)"

#include <ScalpKit/Core.mqh>

input group "=== Strategiya ==="
input int     InpStrategyKind         = 1;    // 0 = pullback, 1 = donchian
input ENUM_TIMEFRAMES InpExpectedTimeframe    = PERIOD_H4; // Preset qaysi TF uchun

input group "=== Rejim filtrlari ==="
input double  InpMinAtrPct            = 0.00324; // ATR% minimal
input double  InpMaxAtrPct            = 0.0252; // ATR% maksimal
...
input double  InpMinSlAtr             = 1.5;  // Stop minimal (ATR)
input double  InpMaxSlAtr             = 3.5;  // Stop maksimal (ATR)
input double  InpMinStopPct           = 0.00288; // Stop minimal (narx %)
input double  InpMaxStopPct           = 0.0432; // Stop maksimal (narx %)
...
input double  InpTp2R                 = 0.0;  // TP2 (R)
...
input double  InpTrailAfterR          = 0.5;  // Trailing boshlanishi (R)
input double  InpTrailAtrMult         = 3.0;  // Trailing masofasi (ATR)
input double  InpTrailMinStepAtr      = 0.25; // Trailing minimal qadami (ATR)
input int     InpTimeStopBars         = 1000000; // Vaqt stopi (bar)
input double  InpTimeStopMinR         = 0.0;  // Vaqt stopi shu R gacha
...
input double  InpExpectedHoldDays     = 2.9167; // Kutilgan ushlash (kun)
...
input long    InpMagic                = 20260908; // Magic raqam
